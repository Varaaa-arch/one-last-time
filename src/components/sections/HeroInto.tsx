...
for (let i = 0; i < GALAXY_COUNT; i++) {
  ...
  galaxyPositions[i3] =
    Math.cos(branch) * radius + (Math.random() > 0.5 ? sx : -sx)

  galaxyPositions[i3 + 1] =
    Math.random() > 0.5 ? sy : -sy

  galaxyPositions[i3 + 2] =
    Math.sin(branch) * radius + (Math.random() > 0.5 ? sz : -sz)
...
function Galaxy() {
  ...
  useFrame((_, delta) => {
    if (mesh.current) {
      mesh.current.rotation.y += delta * 0.018
    }
  })
...
function CameraController({
  ...
  useFrame(() => {
    ...
    camera.position.z +=
      (THREE.MathUtils.lerp(14, 1, t) - camera.position.z) * 0.05

    camera.position.y +=
      (THREE.MathUtils.lerp(2, 0.2, t) - camera.position.y) * 0.05
      ...
      {LINES.map((line, i) => (
        <div
          ...
          style={{
            ...
            padding: "0 2rem",
            textShadow:
              "0 0 40px rgba(120,80,200,0.4), 0 2px 20px rgba(0,0,0,0.8)",
...
export default function HeroIntro() {
  ...
  const handleContinue = () => {
    ...
    lenis?.scrollTo(window.scrollY + 200, {
      duration: 1.2,
    })
